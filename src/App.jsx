...
function DashboardPage({ user, setPage, setShowAuth }) {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "24px 16px" : "40px 24px" }}>
      {/* Header */}   <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
...
function PaymentPage({ plan, user, setPage, setShowAuth }) {
  const isMobile = useIsMobile();
  const [selected, setSelected] = useState(CRYPTO_WALLETS[0]);  const [amount, setAmount] = useState(plan?.min ?? 500);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: isMobile ? "40px 16px" : "60px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 44 }}>
        ...
        <div style={{ color: PALETTE.textMuted, fontSize: 14 }}>Can't find your answer? Contact our 24/7 support team.</div>      </div>
    ...
    <div style={{ background: PALETTE.void, color: PALETTE.text, fontFamily: "-apple-system,'SF Pro Display','Segoe UI',system-ui,sans-serif", minHeight: "100vh", overflowX: "hidden" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        button:active{opacity:.8}        ::-webkit-scrollbar{width:4px;height:4px}
